...
init_from = 'scratch'   # Options : 'scratch', 'resume', 'gpt2*'(disabled!)
# wandb logging
wandb_log = True
...
# Configuration using the global variables above
config_keys = [k for k, v in globals().items() if not k.startswith('_') and isinstance(v, (int, float, bool, str))]
exec(open('configurator.py').read())
config = {k: globals()[k] for k in config_keys}


# Multiprocessing using Distributed Data Parallel (DDP)
...
if ddp:
    ...
    seed_offset = ddp_rank  # Different seed for each process
...
# Crop down if the intended block size is greater than the default (1024)
if block_size < model.config.block_size:
    model.crop_block_size(block_size)
    model_args['block_size'] = block_size
model.to(device)

# Intialize a GradScaler (to prevent the underflow if bf16 is not supported!)
...
checkpoint = None   # Free up the memory
